...
def test_analyze_file_source():
    ...
    result = lcs_id_obj.analyze_file(input_fp=src)
    assert len(result) == 1
    lcs_match, summary_obj = result[0]
    assert summary_obj["matched_license"] == 'test_license'
    assert summary_obj["score"] == 1.0
...
def test_postprocess_comments():
    ...
    postprocess_dict = lcs_id_obj.postprocess_strip_off_comments(result_dict)
    ...
    postprocess_dict = lcs_id_low_threshold.postprocess_strip_off_comments(result_dict)
...
def test_analyze_file_multiple_licenses():
    ...
    result = lid_obj.analyze_file(src)
    assert len(result) == 3
