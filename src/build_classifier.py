...
class NGramBuilder:
    ...
    def measure_Jaccard_distance(self, license_name):
        ...
        if (sum(uni_union.values()) > 0):
            uni_score = sum(uni_intersect.values()) / sum(uni_union.values())
        else:
            uni_score = 0

        if (sum(bi_union.values()) > 0):
            bi_score = sum(bi_intersect.values()) / sum(bi_union.values())
        else:
            bi_score = 0
        if (sum(tri_union.values()) > 0):
            tri_score = sum(tri_intersect.values()) / sum(tri_union.values())
        else:
            tri_score = 0

        return (uni_score + bi_score*6 + tri_score*8 )/16
